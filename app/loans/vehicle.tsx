...
export default function VehicleLoan() {
  ...
  const add50K = () => {
    ...
    const newAmount = Math.min(currentAmount + 50000, 3000000); // Cap at 30L
  ...
  const add1L = () => {
    const currentAmount = parseInt(loanAmount) || 0;
    const newAmount = Math.min(currentAmount + 100000, 3000000); // Cap at 30L
    setLoanAmount(newAmount.toString());
    validateForm(newAmount.toString(), tenure);
  };
  ...
  const validateForm = (amount: string, tenureMonths: string) => {
    const amountValid = parseInt(amount) >= 50000 && parseInt(amount) <= 3000000;
    const tenureValid = parseInt(tenureMonths) >= 12 && parseInt(tenureMonths) <= 84;
  ...
  const handleNext = () => {
    if (isFormValid) {
      router.push({
        ...
        params: { amount: loanAmount, tenure: tenure, type: 'vehicle' }
      });
    }
  };
    ...
    >
      ...
      <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.formContainer}>
          <Text style={styles.sectionTitle}>Let's get you a vehicle loan</Text>
          ...
          <View style={styles.inputSection}>
            ...
            <View style={styles.amountRangeInfo}>
              <Text style={styles.amountRangeText}>Amount Range: ₹50,000 - ₹30,00,000</Text>
            </View>
          </View>
          ...
          >
            <LinearGradient
              colors={isFormValid ? ['#FF3B30', '#FF5E3A', '#FF3B30'] : ['#CCCCCC', '#DDDDDD']}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
