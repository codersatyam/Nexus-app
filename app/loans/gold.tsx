...
export default function GoldLoan() {
  ...
  const add50K = () => {
    ...
    const newAmount = Math.min(currentAmount + 50000, 2000000); // Cap at 20L
  ...
  const add1L = () => {
    const currentAmount = parseInt(loanAmount) || 0;
    const newAmount = Math.min(currentAmount + 100000, 2000000); // Cap at 20L
    setLoanAmount(newAmount.toString());
    validateForm(newAmount.toString(), tenure);
  };
  ...
  const validateForm = (amount: string, tenureMonths: string) => {
    const amountValid = parseInt(amount) >= 20000 && parseInt(amount) <= 2000000;
    const tenureValid = parseInt(tenureMonths) >= 3 && parseInt(tenureMonths) <= 36;
  ...
  const handleNext = () => {
    if (isFormValid) {
      router.push({
        ...
        params: { amount: loanAmount, tenure: tenure, type: 'gold' }
      });
    }
  };
    ...
    >
      ...
      <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.formContainer}>
          <Text style={styles.sectionTitle}>Let's get you a gold loan</Text>
          ...
          <View style={styles.inputSection}>
            ...
            <View style={styles.amountRangeInfo}>
              <Text style={styles.amountRangeText}>Amount Range: ₹20,000 - ₹20,00,000</Text>
            </View>
          </View>
          ...
          >
            <LinearGradient
              colors={isFormValid ? ['#DAA520', '#FFD700', '#DAA520'] : ['#CCCCCC', '#DDDDDD']}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
